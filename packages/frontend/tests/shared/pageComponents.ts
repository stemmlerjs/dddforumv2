...
export class PageComponents {
  ...
  public async load() {
    ...
    const loadedPageComponents: LoadedPageComponents = {};
  ...
  get(name: string) {
    if (!this.loadedPageComponents)
      throw new Error('Page components not loaded yet, make sure you call `.load()` first');
